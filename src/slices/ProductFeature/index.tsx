...
import type { RichTextField, ImageField, ContentRelationshipField, PrismicDocument } from "@prismicio/client";
...
export type ProductFeatureProps = SliceComponentProps<ProductFeatureSlice>;

// Define fragrance document type
type FragranceDocument = PrismicDocument & {
  data: {
    title: string;
    price: number;
    bottle_image: ImageField;
  };
};
...
const ProductFeature: FC<ProductFeatureProps> = async ({ slice }) => {
  ...
  const fragrance = isFilled.contentRelationship(slice.primary.fragrance)
    ? await client.getByID<FragranceDocument>(slice.primary.fragrance.id)
    : null;
          ...
          <div className="flex justify-between p-10 pt-4">
            <div className="space-y-1">
              <h3 className="font-display text-4xl">
                {fragrance?.data.title || "Fragrance"}
              </h3>
